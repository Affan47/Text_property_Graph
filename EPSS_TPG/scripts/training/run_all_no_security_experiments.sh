...
# `outputs/social_media/<llm>/<variant>/` and `outputs/megavul/<llm>/<variant>/`
# to justify (or unjustify) the
# inclusion of the security frontend.
#
# What changes vs the original 35-run script:
#   * Every run adds `--no-security-frontend`.
#   * Outputs go to `outputs/security_ablation/{social_media,megavul,nvd_kev}/...`
#     so they do not
...
LOG_DIR="$ROOT/datasets_info/Security_ablation/run_logs"
mkdir -p "$LOG_DIR"
...
for ds in gpt gemma mistral deepseek; do
    for variant in D S_all S_git S_cvss ALL; do
        ...
        output_dir="outputs/security_ablation/social_media/${ds}/${variant}"
...
# Output under outputs/security_ablation/megavul/<llm>/<variant>/.
for ds in mv_gpt mv_mistral mv_gemma; do
    ...
    for variant in D S_url S_code S_cvss ALL; do
        ...
        csv="${MEGAVUL_CSV[$ds]}"
        data_dir="data/no_security_ablation/${run_id}"
        output_dir="outputs/security_ablation/megavul/${llm}/${variant}"
...
# Output under outputs/security_ablation/nvd_kev/<run_id>/.
EPSS_DIR="data/epss"
...
if [[ -f "$NVD_BAL" ]]; then
    EXPERIMENTS+=("nvd_kev_binary_rerun|labeled|$NVD_BAL|data/no_security_ablation/nvd_kev_binary_rerun|outputs/security_ablation/nvd_kev/binary_rerun|${NVD_BINARY_FLAGS}")
fi
if [[ -f "$TEMPORAL_DIR_22/labeled_cves_temporal_train.json" && -f "$TEMPORAL_DIR_22/labeled_cves_temporal_test.json" ]]; then
    EXPERIMENTS+=("nvd_kev_temporal_with_epss|labeled-temporal|$TEMPORAL_DIR_22|data/no_security_ablation/nvd_kev_temporal_with_epss|outputs/security_ablation/nvd_kev/temporal_with_epss|${NVD_TEMPORAL_WITH_EPSS_FLAGS}")
fi
if [[ -f "$TEMPORAL_DIR_20/labeled_cves_temporal_train.json" && -f "$TEMPORAL_DIR_20/labeled_cves_temporal_test.json" ]]; then
    EXPERIMENTS+=("nvd_kev_temporal_no_epss|labeled-temporal|$TEMPORAL_DIR_20|data/no_security_ablation/nvd_kev_temporal_no_epss|outputs/security_ablation/nvd_kev/temporal_no_epss|${NVD_TEMPORAL_NO_EPSS_FLAGS}")
fi
...
echo "  Output base:               outputs/security_ablation/{social_media,megavul,nvd_kev}/"
