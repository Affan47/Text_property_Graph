...
LOG_DIR="$ROOT/datasets_info/Summary_in_TPG_ablation/run_logs"
mkdir -p "$LOG_DIR"
...
for ds in mv_gpt mv_mistral mv_gemma; do
    # strip the leading "mv_" so the output tree groups by LLM, not by prefix
    llm="${ds#mv_}"
    for variant in D S_url S_code S_cvss ALL; do
        run_id="${ds}_${variant}"
        csv="${MEGAVUL_CSV[$ds]}"
        data_dir="data/epss_${ds}_${variant}"
        output_dir="outputs/megavul/${llm}/${variant}"
