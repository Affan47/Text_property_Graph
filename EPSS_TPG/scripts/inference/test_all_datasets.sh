...
# dataset families:
#
#   1. Social-media (Sec4AI4Aec):  outputs/social_media/<llm>/<variant>/
#   2. Megavul:                    outputs/megavul/<llm>/<variant>/
#   3. NVD/KEV:                    outputs/nvd_kev/<run>/
#
...
#   - outputs/<family>/test_only_summary.csv
#   - outputs/test_only_combined_summary.csv  (after all families finish)
#
# Usage:
...
# Resolve project root from this script's location (scripts/inference/<name>.sh -> ../..)
...
FAMILY_GLOBS["social_media"]="outputs/social_media/*/*"
FAMILY_GLOBS["megavul"]="outputs/megavul/*/*"
FAMILY_GLOBS["nvd_kev"]="outputs/nvd_kev/*"
FAMILY_GLOBS["security_ablation"]="outputs/security_ablation/*"
...
if [[ $# -eq 0 ]]; then
    FAMILIES=("social_media" "megavul" "nvd_kev" "security_ablation")
else
    FAMILIES=("$@")
fi
...
for fam in "${FAMILIES[@]}"; do
    ...
    summary_file="$SUMMARY_DIR/${fam}/test_only_summary.csv"
...
for fam in [$(printf '"%s",' "${RAN_FAMILIES[@]}")]:
    path = Path("$SUMMARY_DIR") / fam / "test_only_summary.csv"
    if not path.exists():
        continue
    df = pd.read_csv(path)
