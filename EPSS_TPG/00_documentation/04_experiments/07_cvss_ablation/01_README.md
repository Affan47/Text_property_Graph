...
**Goal:** Confirm or reject the hypothesis raised by the [TPG ablation](../06_graph_ablation/tpg_ablation_results.md): that the **CVSS components in the tabular branch are the dominant remaining target proxy** driving the model's PR-AUC ≥ 0.97 across all 32 prior hybrid runs.
...
The [TPG ablation](../06_graph_ablation/tpg_ablation_results.md) measured a 0.19-0.63 PR-AUC drop when the tabular branch was disabled (`--hybrid` off). On full data: hybrid Run A = 0.9986 vs TPG-only T1 = 0.8119. On max-clean: hybrid Run H = 0.9739 vs TPG-only T4 = 0.3406.
...
This will then be folded into [OVERALL_ANALYSIS.md](../../../05_results/03_dataset_analysis/OVERALL_ANALYSIS.md) §10 to close out the leakage-source investigation.
