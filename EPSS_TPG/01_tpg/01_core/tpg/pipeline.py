...
class DomainPipeline(TPGPipeline):
    """Generic domain pipeline — Level 2 for ANY domain, declared as data.

    Uses PatternFrontend + a DomainSpec (registered name or instance) so
    that new domains (medical, legal, financial, ...) need zero new code:

        graph = DomainPipeline("medical").run(clinical_note)
        graph = DomainPipeline(DomainSpec.from_json("aviation.json")).run(report)
    """

    def __init__(self, domain="general", model: str = "en_core_web_sm",
                 passes: Optional[List[BasePass]] = None,
                 schema: Optional[TPGSchema] = None):
        from tpg.frontends.pattern_frontend import PatternFrontend
        frontend = PatternFrontend(domain=domain, model=model,
                                   schema=schema or DEFAULT_SCHEMA)
        super().__init__(frontend=frontend, passes=passes,
                         schema=schema or DEFAULT_SCHEMA)
        self.domain = frontend.spec.name
...
def parse_domain_text(text: str, domain: str = "general",
                      doc_id: str = "") -> TextPropertyGraph:
    """One-liner to parse text with a declarative domain overlay."""
    return DomainPipeline(domain=domain).run(text, doc_id=doc_id)
